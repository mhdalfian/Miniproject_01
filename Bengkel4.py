# Import modul os untuk membersihkan layar terminal
import os
# Import class PrettyTable dari modul prettytable
...
class Bengkel:
    ...
    def ubah_motor(self, nomor_urut, atribut, nilai_baru):
        ...
        if motor:
            ...
            # Setelah melakukan perubahan, tampilkan kembali daftar motor yang sudah diubah
            self.lihat_motor()
        else:
    ...
    # Fungsi untuk metode searching memakai jump search berdasar tahun pembuatan
    def jump_search_tahun_pembuatan(self, tahun):
        if not self.head:
            print("Belum ada motor yang terdaftar.")
            return None

        current = self.head
        cari_motor = []  # List untuk menyimpan semua motor yang sesuai dengan kriteria pencarian
        while current and current.tahun_pembuatan <= tahun:
            if current.tahun_pembuatan == tahun:
                cari_motor.append(current)
            current = current.next_motor

        if cari_motor:
            print("Motor ditemukan:")
            table = PrettyTable(["No", "Merek", "Tahun Pembuatan", "CC Motor", "Kondisi", "Terakhir Servis(bulan)"])
            for index, motor in enumerate(cari_motor, 1):
                table.add_row([index, motor.merek, motor.tahun_pembuatan, motor.cc_motor, motor.kondisi, motor.terakhir_servis])
            print(table)
        else:
            print("Motor dengan tahun pembuatan", tahun, "tidak ditemukan.")
        return cari_motor

    # Fungsi untuk metode searching memakai jump search berdasar CC Motor
    def jump_search_cc_motor(self, cc):
        if not self.head:
            print("Belum ada motor yang terdaftar.")
            return None
        ...
        if cari_motor:
            ...
            print("Motor dengan CC motor", cc, "tidak ditemukan.")
        return cari_motor
...
def main():
    ...
    while True:
        ...
        print("7. Search berdasarkan tahun")
        print("8. Search berdasarkan CC Motor")
        print("9. Keluar")

        try:
            pilihan = int(input("Masukkan pilihan (1/2/3/4/5/6/7/8/9): "))  # Menerima input pilihan dari pengguna
        except ValueError:
        ...
        if pilihan == 1:
            ...
            print("===== SEARCHING BERDASARKAN TAHUN PEMBUATAN =====\n")
            try:
                tahun_cari = int(input("Masukkan tahun pembuatan motor yang ingin dicari: "))
            except ValueError:
                print("Masukkan tahun pembuatan yang valid.")
                continue
            
            bengkel.head = bengkel.merge_sort_tahun(bengkel.head, ascending=True)
            bengkel.jump_search_tahun_pembuatan(tahun_cari)
            
        elif pilihan == 8:
            print("===== SEARCHING BERDASARKAN CC MOTOR =====\n")
            try:
                cc_cari = int(input("Masukkan CC motor yang ingin dicari: "))
            except ValueError:
                print("Masukkan CC motor yang valid.")
                continue
            
            bengkel.jump_search_cc_motor(cc_cari)
